...
const QuotationComponent = ({ productDetails, onClose, selectedLead }) => {
  ...
  const handleImageClick = (rowId) => {
    ...
    if (selectedProduct.image) {
      setModalImage(selectedProduct.image); // Set the image to display.
      setOpenModal(true);
    } else {
    ...
    <div className="quotation-container  p-4 rounded-lg  max-h-[100vh] overflow-y-auto scrollbar-thin">
      ...
      <div className="flex items-center  justify-between">
        ...
        <div className="flex gap-4 justify-between items-center">
          <button
            onClick={togglePopup}
            className=" px-4 py-1  rounded-md text-white bg-green-500 hover:bg-green-700 transition"
          >
            Save
          </button>
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        ...
        <TextField
          label="Coordinator"
          value={selectedLead.coordinator}
          size="small"
        />
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        <div className="flex flex-col gap-2">
          ...
          {/* <TextField label="Address" value="12050458" size="x-large" /> */}
          ...
          <TextField
            ...
            value={[
              ...
              selectedLead.city,
              selectedLead.apartment,
              selectedLead.areaName,
              selectedLead.location,
            ]
              .filter((field) => field && field.trim() !== "") // Exclude empty or whitespace-only fields
              .join(", ")} // Join the fields with a comma and space
            size="x-large"
            fullWidth
            multiline
            variant="outlined"
            InputProps={{
              style: { fontSize: "1rem" }, // Adjust font size for better readability
            }}
          />
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <QuotationDocs />
            <RelatedDocs selectedLead={selectedLead} />
      ...
      <Box>
        ...
        >
          {selectedTab === 0 && (
            <div>Delivery details: Delivered within 7 business days.</div>
          )}
          {selectedTab === 1 && (
            <div>Payment terms: 50% advance, 50% upon delivery.</div>
          )}
          {selectedTab === 2 && (
            <div>Validity: Quotation valid for 30 days from issuance.</div>
          )}
          {selectedTab === 3 && (
            <div>
              Remarks: Please ensure all details are accurate before confirming.
            </div>
          )}
          {selectedTab === 4 && (
            <div>Terms: Standard terms and conditions apply.</div>
          )}
        </Box>
      </Box>
      ...
      <ImageModal
        ...
        onImageChange={(e) => {
          ...
          if (file) {
            ...
            setModalImage(newImageUrl); // Update the modal image

            // Update the product image in the products state
            setProducts((prevProducts) =>
              prevProducts.map((product) =>
                product.image === modalImage // Find the product with the matching image
                  ? { ...product, image: newImageUrl } // Update its image
                  : product
              )
            );
          }
